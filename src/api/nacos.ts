...
class NacosApi {
  ...
  async getServiceList(
    ...
    return this.instance.get<
      NacosResponse<{ count: number; doms: ServiceInfo[] }>
    >("/catalog/services", {
      params,
    });
  }
  ...
  async getServiceDetail(serviceName: string, groupName?: string) {
    ...
    return this.instance.get<NacosResponse<ServiceInstance[]>>(
      "/catalog/instances",
      {
        params,
      },
  ...
  async getConfigList(
    ...
    return this.instance.get<
      NacosResponse<{ pageItems: ConfigItem[]; totalCount: number }>
    >("/cs/configs", {
      params,
    });
  }
  ...
  async deleteNamespace(namespaceId: string) {
    return this.instance.delete<NacosResponse>(
      `/console/namespaces/${namespaceId}`,
    );
  }

  // 集群管理相关API
  async getClusterList() {
    return this.instance.get<NacosResponse<Record<string, unknown>>>(
      "/cluster/list",
    );
  }
  ...
  async login(username: string, password: string) {
    return this.instance.post<
      NacosResponse<{ accessToken: string; tokenTtl: number }>
    >("/auth/login", {
      username,
      password,
    });
