<template>
  <div class="cluster-nodes-page">
    ...
    <el-card class="table-card">
      <el-table v-loading="nacosStore.loading" :data="clusterNodes" stripe style="width: 100%">
        ...
        <el-table-column prop="failAccessCnt" label="失败次数" width="120" align="center">
          <template #default="{ row }">
            <el-tag :type="row.failAccessCnt > 0 ? 'danger' : 'success'" size="small">
              {{ row.failAccessCnt }}
            </el-tag>
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" width="150" fixed="right">
          <template #default="{ row }">
            ...
            <el-button type="warning" size="small" @click="pingNode(row)"> Ping </el-button>
          </template>
        </el-table-column>
      </el-table>
    </el-card>
      ...
      <div v-if="currentNode" class="node-detail">
        <el-descriptions :column="2" border>
          ...
          <el-descriptions-item label="失败次数">
            <el-tag
              :type="(Number(currentNode.failAccessCnt) || 0) > 0 ? 'danger' : 'success'"
              size="small"
            >
    ...
    <!-- 集群指标对话框 -->
    <el-dialog v-model="showMetricsDialog" title="集群指标" width="80%" top="5vh">
...
const healthyNodes = computed(() => clusterNodes.value.filter((node) => node.state === 'UP').length)

const warningNodes = computed(
  () => clusterNodes.value.filter((node) => node.state === 'SUSPICIOUS').length,
)

const errorNodes = computed(() => clusterNodes.value.filter((node) => node.state === 'DOWN').length)
